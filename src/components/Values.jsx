...
const Values = () => {
  return (
    <div id='about' className='pt-[156px] max-md:pt-28 max-sm:pt-[120px] overflow-hidden'>
        ...
        <div className='flex flex-wrap pt-[77px] max-md:pt-8 max-sm:pt-10 -mx-2 justify-center'>
          {VALUES_LIST.map((obj, i) => (
            <div key={i} className='w-1/3 max-md:w-1/2 max-sm:w-full relative group px-2 max-md:pt-3'>
              <img src={moon} alt="moon" className='absolute -top-[19px] -right-[23px] pointer-events-none h-[74px] opacity-0 group-hover:opacity-100 group-hover:z-10' />
              <div className='p-5 box-blur transition-all cursor-pointer h-full duration-300 border border-gray hover:border-opacity-10 hover:z-30 relative z-0 hover:border-white rounded-xl hover:shadow-[0px_-1px_20px_0px] hover:shadow-lightGray hover:bg-white hover:bg-opacity-10'>
                <div>{obj.icon}</div>
