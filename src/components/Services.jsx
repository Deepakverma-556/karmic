...
const Services = () => {
  return (
    <div id='services' className='max-md:pt-16 max-sm:pt-20 relative max-w-[1920px] mx-auto'>
      <CustomShadow myClass="left-0 bottom-0" />
      <div className='max-w-[1172px] mx-auto px-4 max-lg:overflow-hidden'>
        <Heading text="services" image={titleline} imageAlt="title-line" />
        <div className='flex flex-wrap pt-[77px] max-sm:pt-10 -mx-2 max-lg:justify-center'>
          {SERVICES_LIST.map((obj, i) => (
            <div key={i} className='w-1/3 max-md:w-1/2 max-sm:w-full group relative px-2'>
              ...
              <div className='p-5 transition-all cursor-pointer h-full duration-300 border border-transparent hover:border-white hover:border-opacity-10 rounded-xl hover:shadow-[0px_-1px_20px_0px] hover:shadow-lightGray hover:bg-white hover:bg-opacity-10 box-blur'>
                <div>{obj.icon}</div>
