...
const Services = () => {
  return (
    <div id='services' className='pb-[136px] max-md:py-16 max-sm:pb-0 max-sm:pt-20 relative max-w-[1440px] mx-auto'>
      ...
      <img src={moon} alt="moon" className='absolute left-[58%] top-[13%] pointer-events-none' />
        ...
        <div className='flex flex-wrap pt-[77px] max-sm:pt-10 -mx-2'>
          {SERVICES_LIST.map((obj, i) => (
            <div key={i} className='w-1/3 max-md:w-1/2 max-sm:w-full group relative px-2'>
              <div className='p-5 relative transition-all duration-300 border border-transparent hover:border-white hover:border-opacity-10 rounded-xl hover:shadow-[0px_-1px_20px_0px] hover:shadow-lightGray hover:bg-white hover:bg-opacity-10 box-blur'>
                <div>{obj.icon}</div>
